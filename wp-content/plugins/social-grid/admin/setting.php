...
$mcode = 0;
$error = false;
if(isset($_POST['create_spost_grid_title'])){
	extract($_POST);
	if($anew_slider_title == '' || empty($anew_slider_title)){
		...
		$wpdb->insert( self::$table_prefix.self::TABLE_SLIDERS_NAME, $ins );
		wp_redirect( 'admin.php?page=ssocial-grid' );exit;
	}
}
...
if(isset($_POST['spost_Update_Setting'])){
	...
	$wpdb->update( self::$table_prefix.self::TABLE_SLIDERS_NAME, $ins, array('id' => $_GET['sid']) );
	wp_redirect( 'admin.php?page=ssocial-grid&view=setting&sid='.$_GET['sid'] );exit;
}
?>
<style type="text/css">
...
<div class="meta-box-sortables ui-sortable">
	<div class="postbox" style="margin-bottom:10px;">
		<div class="inside" style="padding:0 12px;">
			<h3 class="h2_logo"><a href="<?php echo self::plugin_root_url();?>" style="text-decoration:none; color:#222;"><?php echo esc_html( 'Social Grid and Carousel' ); ?></a></h3>
		</div>
	</div>
</div>
...
$difault_grid_array = array(
    'title' => '',
    'grid_type' => 'post_layout',
    'post_type' => array('social'),
    'post_count' => '10',
    'order_by' => 'date',
    'order' => 'DESC',
	'social_type' => array('facebook','twitter','youtube','vimeo','instagram','dribbble','pinterest','vk','blockquote'),
    'skin_type' => 's1',
    'car_display_item' => '4',
    'car_autoplay_time' => '5',
    'synced_display' => '10',
...
<div style="float:right; margin-top:10px; clear:both; font-weight:bold;">Social Grid Version <?php echo $anisliderVersion;?></div>
</div>
